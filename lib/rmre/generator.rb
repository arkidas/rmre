...
module Rmre
  class Generator
    ...
    def create_model(table_name)
      table_name_singular = "#{table_name.tableize.singularize}.rb"

      if /^[\d]+(\.[\d]+){0,1}$/ === table_name_singular[4]
        table_name_singular.sub! 'afp_', 'afp'
      end

      File.open(File.join(output_path, table_name_singular), "w") do |file|
        constraints = []
